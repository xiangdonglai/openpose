...
int CSyncManager::ConvFrameToTC_vga(FILE* fp_vga,int vgaFrameIdx)     //FrameIdx starts from 0
{
    ...
    const auto result = fread(im_vga, VGA_TIMECODE_SIZE,1,fp_vga);
    (void)result;
...
void CSyncManager::ConvFrameToTCStr_hd(FILE* fp,off64_t iframe,char *timeCodeStr)
{
    if(fp)
    {
        const auto ret = fseeko64(fp, iframe*(HD_BLOCKSIZE_WITHHEAD)+HD_BLOCKSIZE, SEEK_SET);
        (void)ret;
        if(timeCodeStr)
        {
            const auto result = fread(timeCodeStr, HD_STRBLOCKSIZE, 1, fp);
            (void)result;
        }
    }
}
